...
private:
  ...
  bool checkHVStatus()
  {
    ...
    // shutdown the HV is if temperature shutdown flag is set
  ...
  {
    char buffer[20];
    sprintf(buffer, "%.2f", _clock.getBoardTemperature());
    _displayHandler.show(buffer);
  }
  ...
  bool handleShortcuts(uint8_t keyCode)
  {
    ...
    case KEY_MS:
      ...
      case device_mode::calculator:
        // store the fixed decimals and the input direction in setting in NVS
        _settings.setSetting(setting_id::fixeddecimals, SettingsCache::fixedDecimals);
        _settings.setSetting(setting_id::calcinputdirec, SettingsCache::calcInputDirec);
        _settings.storeSettings();
        break;
