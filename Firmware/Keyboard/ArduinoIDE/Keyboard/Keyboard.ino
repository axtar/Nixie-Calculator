...
// Dependencies: https://github.com/chris--a/keypad

// the keyboard receives commands from the controller using I2C
// key events are queued locally and sent to the controller on I2C request
// the KINT pin signals whether a event is queued
...
// size of key event served over I2C
constexpr size_t KEY_INFO_SIZE = 2;

// depth of the pending key event queue
constexpr uint8_t KEY_QUEUE_SIZE = 8;
...
// a single queued key event, served to the controller on I2C request
typedef struct
{
  uint8_t keyCode;
  uint8_t state;
...
// pending key event queue, shared between loop() and the I2C request ISR
volatile KEY_EVENT keyQueue[KEY_QUEUE_SIZE];
volatile uint8_t keyQueueHead = 0; // next slot to pop
volatile uint8_t keyQueueTail = 0; // next slot to push
volatile uint8_t keyQueueCount = 0;
...
void queueKeyEvent(uint8_t keyCode, uint8_t state);
void updateDataReadyPin();
...
void setup()
{
  ...
  // queue starts empty
  pinMode(PIN_KINT, OUTPUT);
  digitalWrite(PIN_KINT, LOW);
  ...
  if (keypad.getKeys())
  {
    for (uint8_t i = 0; i < LIST_MAX; i++)
    {
      if (keypad.key[i].stateChanged)
      {
        ...
        __serial_print("Millis: ");
        __serial_print(millis());
        __serial_print(" ");
        __serial_print("Key: ");
        __serial_print((int)keypad.key[i].kchar);
        __serial_print(" ");
        __serial_print("State: ");
        __serial_println(keypad.key[i].kstate);

        // keypad state changed, queue key and state 
        queueKeyEvent((uint8_t)keypad.key[i].kchar, (uint8_t)keypad.key[i].kstate);
    ...
    for (uint8_t i = 0; i < LIST_MAX; i++)
    {
      ...
      if (keyHoldInfo[i].keyCode != 0)
      {
        ...
        if (genEvent)
        {
          ...
          __serial_print(" ");
          __serial_print("Key: ");
          __serial_print(keyHoldInfo[i].keyCode);
          __serial_print(" ");
          __serial_print("State: ");
          __serial_println(KEYSTATE_AUTOREPEAT);

          queueKeyEvent(keyHoldInfo[i].keyCode, KEYSTATE_AUTOREPEAT);
...
void requestEvent()
{
  ...
  default:
    // no command, just send the oldest key event if any
    if (keyQueueCount > 0)
    {
      Wire.write(keyQueue[keyQueueHead].keyCode);
      Wire.write(keyQueue[keyQueueHead].state);
...
// push a key event onto the queue
void queueKeyEvent(uint8_t keyCode, uint8_t state)
{
  noInterrupts();
  ...
  keyQueue[keyQueueTail].keyCode = keyCode;
  keyQueue[keyQueueTail].state = state;
...
// inform about queue state 
void updateDataReadyPin()
{
  digitalWrite(PIN_KINT, keyQueueCount > 0 ? HIGH : LOW);
}
...
void onSetAutoRepeatInterval()
{
  ...
  initKeyHoldInfo();
  __serial_println(autoRepeatInterval);
}
