...
// minimum payload lengths required for fixed-offset field extraction,
// a shorter payload would read past the allocated payload buffer
constexpr uint16_t UBX_NAV_STATUS_MIN_LEN = 6;
constexpr uint16_t UBX_MON_VER_MIN_LEN = 40;
constexpr uint16_t UBX_NAV_TIMEUTC_MIN_LEN = 20;
...
public:
  ...
  void process()
  {
    ...
    else
    {
      if (_verbose)
      {
        __serial_println("Com port not defined. Call begin first");
      }
    }
  }
...
private:
  ...
  void onNack(const UBXMESSAGE *message)
  {
    ...
    if (_verbose)
    {
      __serial_println("Received nack");
    }
  }
  ...
  void onStatus(const UBXMESSAGE *message)
  {
    ...
    if (_verbose)
    {
      __serial_print("Time of week:        ");
      __serial_println(_gpsStatus.timeOfWeek);
      __serial_print("GPS fix type:        ");
      __serial_println(_gpsStatus.gpsFixType);
      __serial_print("GPS fix  OK:         ");
      __serial_println(_gpsStatus.gpsFixOk);
      __serial_print("Corrections applied: ");
      __serial_println(_gpsStatus.diffApplied);
      __serial_print("ToW valid:           ");
      __serial_println(_gpsStatus.timeOfWeekValid);
      __serial_print("Week number valid:   ");
      __serial_println(_gpsStatus.weekNumberValid);
    }
  }
  ...
  void onVersion(const UBXMESSAGE *message)
  {
    ...
    if (_verbose)
    {
      __serial_print("Software version: ");
      __serial_println(_moduleVersion.swVersion);
      __serial_print("Hardware version: ");
      __serial_println(_moduleVersion.hwVersion);
      for (uint8_t i = 0; i < MAX_EXTENSIONS; i++)
      {
        __serial_printf("Extension %u: ", i + 1);
        __serial_println(_moduleVersion.extensions[i]);
      }
    }
  }
  ...
  void onTimeUTC(const UBXMESSAGE *message)
  {
    ...
    {
      __serial_print("Time of week:       ");
      __serial_println(_timeUTC.timeOfWeek);
      __serial_print("accuracy:           ");
      __serial_println(_timeUTC.accuracy);
      __serial_print("Nanoseconds:        ");
      __serial_println(_timeUTC.nanoSecond);
      __serial_print("Year:               ");
      __serial_println(_timeUTC.year);
      __serial_print("Month:              ");
      __serial_println(_timeUTC.month);
      __serial_print("Day:                ");
      __serial_println(_timeUTC.day);
      __serial_print("Hour:               ");
      __serial_println(_timeUTC.hour);
      __serial_print("Minute:             ");
      __serial_println(_timeUTC.minute);
      __serial_print("Second:             ");
      __serial_println(_timeUTC.second);
      __serial_print("Time of week valid: ");
      __serial_println(_timeUTC.timeOfWeekValid);
      __serial_print("Week number valid:  ");
      __serial_println(_timeUTC.weekNumberValid);
      __serial_print("UTC valid:          ");
      __serial_println(_timeUTC.utcValid);
      __serial_print("Timestamp:          ");
      __serial_println(_timeUTC.timestamp);
    }
  }
  ...
  void processMessage(const UBXMESSAGE *message)
  {
    ...
    // validate checksum for every message before reading any field from it
    bool corrupted = !validateChecksum(message);
    if (!corrupted)
    {
      ...
      case UBX_MON:
        ...
        {
        case UBX_MON_VER:
          // validate payload length before reading fixed offsets
          if (message->payloadLength >= UBX_MON_VER_MIN_LEN)
          {
            onVersion(message);
          }
          else
      ...
      case UBX_NAV:
        ...
        case UBX_NAV_STATUS:
          // validate payload length before reading fixed offsets
          if (message->payloadLength >= UBX_NAV_STATUS_MIN_LEN)
          {
            onStatus(message);
          }
          else
        ...
        case UBX_NAV_TIMEUTC:
          // validate payload length before reading fixed offsets
          if (message->payloadLength >= UBX_NAV_TIMEUTC_MIN_LEN)
          {
            onTimeUTC(message);
          }
          else
