...
constexpr uint8_t KEY_MINUS = 33; // subtraction
...
// size of key info delivered over I2C
constexpr uint8_t KEY_INFO_SIZE = 2;
...
public:
  ...
  // define the keyboard data-ready pin
  void begin(uint8_t dataReadyPin)
  {
    _dataReadyPin = dataReadyPin;
    pinMode(_dataReadyPin, INPUT_PULLDOWN);
  }
  ...
  // mark activity from a source other than the physical keyboard
  ...
  // get queued key events from the keyboard
  void process()
  {
    while (digitalRead(_dataReadyPin) == HIGH)
    {
      Wire.requestFrom(KEYBOARD_I2C_ADDRESS, (int)KEY_INFO_SIZE);
      if (Wire.available() < KEY_INFO_SIZE)
      {
      ...
      if (_notifyRaw)
      {
        _notifyRaw(keyCode, (key_state)state);
      }
      if (_notify)
      {
        notifyKeyboardEvent(keyCode, (key_state)state);
      }
    }
  }
