...
public:
  ...
  bool initialize()
  {
    ...
    if (_uGPS.isInitialized())
    {
      ...
      _uGPS.attach([this](const UBXMESSAGE *message)
                   { onGPSMessage(message); });

      // print some GPS module information
      __serial_println();
      __serial_println("GPS module successfully initialized");
      __serial_print("Software version: ");
      __serial_println(_uGPS.getModuleVersion().swVersion);
      __serial_print("Hardware version: ");
      __serial_println(_uGPS.getModuleVersion().hwVersion);
      for (uint8_t i = 0; i < MAX_EXTENSIONS; i++)
      {
        __serial_printf("Extension %u: ", i + 1);
        __serial_println(_uGPS.getModuleVersion().extensions[i]);
      }
      _initialized = true;
      retVal = true;
    }
    ...
    {
      __serial_println("GPS not initialized");
    }
    return (retVal);
  }
  ...
  // return the module's software/hardware version information
  MODULEVERSION getModuleVersion() const
  {
    return (_uGPS.getModuleVersion());
  }

  // return the millis() timestamp of the last successful time sync
  unsigned long getLastSyncMillis() const
  {
    return (_lastSyncMillis);
  }

  // called by the owner once the internal clock has actually been synced
  void notifySynced()
  {
  ...
  void gpsTimeSync(TIMEUTC timeUTC)
  {
    ...
    if (millis() - _gpsSyncTimestamp > (_gpsSyncIntervalActive))
    {
      ...
      if (timeUTC.timeOfWeekValid)
      {
        ...
        tm.tm_sec = timeUTC.second;
        tm.tm_min = timeUTC.minute;
        tm.tm_hour = timeUTC.hour;
      }
      if (timeUTC.weekNumberValid)
      {
        tm.tm_mday = timeUTC.day;
        tm.tm_mon = timeUTC.month - 1;
        tm.tm_year = timeUTC.year - 1900;
      }
