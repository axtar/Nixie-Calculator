...
public:
  ...
  // return if a calculation could be slow
  ...
  bool isErrorRecoveryOperation(operation op) const
  {
    bool result = false;
    switch (op)
    {
  ...
  // get memory register
  PRAT getMemReg(uint8_t index) const
  {
    if (index < MEM_REGISTER_COUNT)
    {
  ...
  String getRatString(PRAT p, NumberFormat format = NumberFormat::Float) const
  {
    String s;
  ...
  void getRegisters(REGISTERMAP &regmap) const
  {
    regmap["X:"] = _regX;
    regmap["Y:"] = _regY;
    regmap["T:"] = _regT;
...
private:
  ...
  void onMemRegOperation(operation op)
  {
    ...
    case operation::memory_addition:
      DUPRAT(p, _regX);
      _operationReturnCode = CalcMath::calculate(&p, getMemReg(0), operation::addition, _radix, _precision);
      setMemReg(p, 0);
      destroyrat(p);
      break;
  ...
  void notifyRegisterUpdate()
  {
    if (_notifyRegisterUpdate)
    {
      __serial_println("notifyStackUpdate");
      if (_operationReturnCode != operation_return_code::success)
      {
        PRAT p = nullptr;
        _notifyRegisterUpdate("X:", p);
  ...
  void notifyMemRegUpdate()
  {
    if (_notifyRegisterUpdate)
    {
      __serial_println("notifyMemRegUpdate");
      for (uint8_t i = 0; i < MEM_REGISTER_COUNT; i++)
      {
  ...
  void printStack() const
  {
    ...
    printRat(_regX);
    __serial_print("Heap:   ");
    __serial_println(esp_get_free_heap_size());
    __serial_print("MinHeap:");
    __serial_println(esp_get_minimum_free_heap_size());
  }
  ...
  {
    __serial_println(std::string(RatToString(p, NumberFormat::Float, _radix, _precision)).c_str());
  }
};
