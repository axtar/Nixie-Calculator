...
public:
  ...
  virtual ~Calculator()
  {
    ...
    for (const auto &value : _lastNotifiedValues)
    {
      PRAT p = value.second;
      destroyrat(p);
    }
  ...
  void getRegisterStrings(REGISTERSTRINGMAP &regStringMap) const
  {
    REGISTERMAP regMap;
  ...
  String getErrorText(operation_return_code code) const
  {
  ...
  void switchRoundingMode()
  {
    SettingsCache::roundingMode = (SettingsCache::roundingMode == rounding_mode::five_four)
                                      ? rounding_mode::cut
                                      : rounding_mode::five_four;
    if (!_inputPending)
    {
      updateNumber();
    }
  ...
  void attachRegisterUpadteCb(notifyRegisterUpdateCb callBack)
  {
    ...
    _calcEngine.attachNotifyRegisterUpdateCb([this](String regId, PRAT p)
                                             { onRegisterUpdate(regId, p); });
  }
  ...
  void onRegisterUpdate(String regId, PRAT p)
  {
    if (p)
    {
      ...
      if (it == _lastNotifiedValues.end())
      {
        PRAT copy = nullptr;
        DUPRAT(copy, p);
        _lastNotifiedValues[regId] = copy;
      }
      else
      {
        DUPRAT(it->second, p);
  ...
  String getCalculatorErrorText() const
  {
  ...
  // get reg X string
  String getResultString(NumberFormat format = NumberFormat::Float) const
  {
...
private:
  ...
  std::map<String, PRAT> _lastNotifiedValues;
