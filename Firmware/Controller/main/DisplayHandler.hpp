...
public:
  ...
  uint8_t _lastTimeDigit[2][6] = {{DIGIT_OFF, DIGIT_OFF, DIGIT_OFF, DIGIT_OFF, DIGIT_OFF, DIGIT_OFF},
                                  {DIGIT_OFF, DIGIT_OFF, DIGIT_OFF, DIGIT_OFF, DIGIT_OFF, DIGIT_OFF}};

  int _lastRolledMinute[2] = {-1, -1};

  // set a clock digit
  void setTimeDigit(uint8_t digit, uint8_t value, digit_content content, uint8_t &lastValue, bool isMinuteBoundary = false)
  {
    if (lastValue == value)
    {
      if (isMinuteBoundary && (SettingsCache::timeEffects == time_effects::rolling2))
      {
        setDigitFaded(digit, value, DIGIT_ROLL_DURATION_MS, time_effects::rolling2, content, value, true);
        return;
      }
      if (getDigit(digit) != value)
      {
        setDigit(digit, value, content);
      }
      else
      {
        setDigitContent(digit, content);
    ...
    switch (SettingsCache::timeEffects)
    {
    case time_effects::crossfade:
      setDigitFaded(digit, value, DIGIT_FADE_DURATION_MS, time_effects::crossfade, content, previousValue);
      break;

    case time_effects::rolling1:
      // rolls every digit on every change
      setDigitFaded(digit, value, DIGIT_ROLL_DURATION_MS, time_effects::rolling1, content, previousValue);
      break;

    case time_effects::rolling2:
      // rolls every digit unconditionally, but only on the tick where the minute changes
      if (isMinuteBoundary)
      {
        setDigitFaded(digit, value, DIGIT_ROLL_DURATION_MS, time_effects::rolling2, content, previousValue);
      }
      else
      {
        setDigit(digit, value, content);
    ...
    case time_effects::rolling3:
      // every digit only rolls when transitioning to 0
      if (value == 0)
      {
        setDigitFaded(digit, value, DIGIT_ROLL_DURATION_MS, time_effects::rolling3, content, previousValue);
      }
      else
      {
        setDigit(digit, value, content);
                ...
                bool showSeconds, bool space, time_separator::time_separator timeSeparator, bool dual = false)
  {
    ...
    uint8_t *lastDigit = _lastTimeDigit[dual ? 1 : 0];
    int &lastRolledMinute = _lastRolledMinute[dual ? 1 : 0];
    bool isMinuteBoundary = (tm->tm_sec == 0) && (tm->tm_min != lastRolledMinute);
    if (isMinuteBoundary)
    {
      lastRolledMinute = tm->tm_min;
    }
    ...
    currDigit++;
    setTimeDigit(currDigit, minutes10, dc, lastDigit[2], isMinuteBoundary);
    currDigit++;
    setTimeDigit(currDigit, minutes01, dc, lastDigit[3], isMinuteBoundary);
    ...
    if (showSeconds)
    {
      setTimeDigit(currDigit, seconds10, dc, lastDigit[4], isMinuteBoundary);
      currDigit++;
      setTimeDigit(currDigit, seconds01, dc, lastDigit[5], isMinuteBoundary);
    }
  }
