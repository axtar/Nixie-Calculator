...
private:
  ...
  // show board (RTC sensor) and MCU (chip die) temperature, separated by a blank tube
  void showBoardTemperature()
  {
    ...
    float mcuTempC = temperatureRead();
    float mcuTemp = (SettingsCache::temperatureCF == temperature_cf::celsius) ? mcuTempC : Helper::celsiusToFahrenheit(mcuTempC);
    char buffer[20];
    sprintf(buffer, "%.2f %.2f", boardTemp, mcuTemp);
    _displayHandler.show(buffer);
  }
  ...
  String buildSnapshotText() const
  {
    ...
    {
      text.concat(_displayHandler.isMinusSignOn() ? '-' : (_displayHandler.isPlusSignOn() ? '+' : ' '));
    }
    text.concat('|');
    ...
    {
      text.concat(_displayHandler.isDigitOn(i) ? static_cast<char>('0' + _displayHandler.getDigit(i)) : ' ');
    }
    text.concat('|');
    for (uint8_t i = 0; i < digitCount; i++)
    {
      text.concat((_displayHandler.getDecimalSeparator(i + dspOffset) == display_state::on) ? '1' : '0');
    }
    text.concat('|');
  ...
  {
    _web.updateClient("S:", buildSnapshotText(), id);
  }
  ...
  void broadcastDisplaySnapshot()
  {
    ...
    if (_web.isInitialized() && (_web.getClientCount() > 0))
    {
      _web.updateClients("S:", buildSnapshotText());
    }
  }
