...
constexpr unsigned long TIME_SYNC_INTERVAL = 5 * 60 * 1000; // ms
constexpr time_t SECONDS_PER_MONTH = 30 * 86400;            // used to scale the RTC drift correction setting
...
{
public:
  Clock(DisplayHandler *displayHandler, Settings *settings)
      : _settings(settings),
        _displayHandler(displayHandler)
  {
    _timeZone = new Timezone(SettingsCache::dstRule, SettingsCache::stdRule);
    _timeZone2 = new Timezone(SettingsCache::dstRule2, SettingsCache::stdRule2);
  ...
  void begin()
  {
    setTimeZones();
    _rtc.begin();
    // seed the system clock from the RTC immediately
    syncTimeFromRTC(true);
  }
  ...
  // reload time zone rules from settings; call after settings are changed
  void setTimeZones()
  {
    _timeZone->defineRules(SettingsCache::dstRule, SettingsCache::stdRule);
    _timeZone2->defineRules(SettingsCache::dstRule2, SettingsCache::stdRule2);
  }

  // periodically re-sync the system clock from the RTC
  void syncTimeFromRTC(bool force = false)
  {
    if (force || millis() - _lastTimeSyncMillis > TIME_SYNC_INTERVAL)
    {
      time_t t = _rtc.get();
      if (t != 0)
      {
  ...
  bool process(const struct tm *tm, const struct tm *tm2)
  {
  ...
  // update the RTC time
  void setSystemAndRTCTime(time_t utc, bool updateDriftReference = true)
  {
    struct timeval tv = {.tv_sec = utc, .tv_usec = 0};
    settimeofday(&tv, nullptr);
    _rtc.set(utc);
    if (updateDriftReference)
    {
      // remember when the RTC was deliberately set, used as the reference point for drift correction
      _settings->setRtcRefEpoch(utc);
    }
  }
  ...
  void onKeyboardEvent(uint8_t keyCode, key_state keyState, bool functionKeyPressed, bool shiftKeyPressed)
  {
    ...
    if (keyState == key_state::pressed)
    {
      ...
      case key_function_type::numericx2:
        ...
        {
          numericInput(clock_mode::stopwatch);
        }
        else
        {
          numericInput(digit);
  ...
  // return current time
  void getCurrentTime(struct tm *tm, struct tm *tm2) const
  {
    time_t utc, local;
    utc = time(nullptr);
    ...
    // to time elements
    gmtime_r(&local, tm);
    // convert to dual time
    local = _timeZone2->toLocal(utc);
    // to time elements
    gmtime_r(&local, tm2);
  }
...
private:
  ...
  Settings *_settings;
  Timezone *_timeZone;
  Timezone *_timeZone2; // time zone for dual time
  ...
  bool _hourParamSet;
  unsigned long _lastTimeSyncMillis = 0;
  bool _timerEnded;
  bool _timerNotifyOn;
  int _lastColorChangeSecond = -1;
  int _lastColorChangeMinute = -1;
  int _lastColorChangeHour = -1;
  int _lastTimerColorSecond = -1;
  int _lastTimerColorMinute = -1;
  int _lastTimerColorHour = -1;
  ...
  {
    if (SettingsCache::rtcDriftCorr == 0)
    {
      return (0);
    }
    time_t refEpoch = _settings->getRtcRefEpoch();
    if ((refEpoch == 0) || (rtcNow <= refEpoch))
    {
      return (0);
    }
    time_t elapsed = rtcNow - refEpoch;
    return (static_cast<time_t>((static_cast<int64_t>(SettingsCache::rtcDriftCorr) * static_cast<int64_t>(elapsed)) / SECONDS_PER_MONTH));
  }
  ...
  // true if a color-change trigger should fire at all right now
  bool colorChangeTriggerActive() const
  {
    if (SettingsCache::trigColorChange == trig_color_change::off)
    {
      return false;
    }
  ...
  // trigger a lighting refresh when a random RGB mode is active
  void checkColorChangeTrigger(const struct tm *tm)
  {
    if (!colorChangeTriggerActive())
    {
      return;
    }
    ...
    if (SettingsCache::trigColorChange == trig_color_change::every_second)
    {
      if (tm->tm_sec != _lastColorChangeSecond)
      {
        _lastColorChangeSecond = tm->tm_sec;
        _refreshLighting = true;
      }
    }
    else if (SettingsCache::trigColorChange == trig_color_change::every_minute)
    {
      if (tm->tm_min != _lastColorChangeMinute)
      {
        _lastColorChangeMinute = tm->tm_min;
        _refreshLighting = true;
      }
    }
    else if (SettingsCache::trigColorChange == trig_color_change::every_hour)
    {
      if (tm->tm_hour != _lastColorChangeHour)
      {
        _lastColorChangeHour = tm->tm_hour;
        _refreshLighting = true;
      }
    }
  }

  // trigger on elapsed second/minute/hour boundaries used by timer and stopwatch,
  void checkElapsedColorChangeTrigger(bool isRunning, unsigned long second, unsigned long minute, unsigned long hour,
                                      int &lastSecond, int &lastMinute, int &lastHour)
  {
    if (!isRunning || !colorChangeTriggerActive())
    {
      return;
    }
    ...
    if (SettingsCache::trigColorChange == trig_color_change::every_second)
    {
      if (static_cast<int>(second) != lastSecond)
      {
        lastSecond = second;
        _refreshLighting = true;
      }
    ...
    {
      if (static_cast<int>(minute) != lastMinute)
      {
        lastMinute = minute;
        _refreshLighting = true;
      }
    ...
    {
      if (static_cast<int>(hour) != lastHour)
      {
        lastHour = hour;
        _refreshLighting = true;
      }
  ...
  void displayTime(const struct tm *tm, const struct tm *tm2)
  {
    switch (SettingsCache::clockMode)
    {
    case clock_mode::time:
  ...
  void showTimeDefault(const struct tm *tm) const
  {
    _displayHandler->showTime(tm, 3, SettingsCache::hourMode, SettingsCache::leadingZero, true, true, SettingsCache::timeSeparator);
  }

  // display the time, clock mode 1 = time, no seconds
  void showTimeNoSeconds(const struct tm *tm) const
  {
    _displayHandler->showTime(tm, 4, SettingsCache::hourMode, SettingsCache::leadingZero, false, true, SettingsCache::timeSeparator);
  }

  // display the time, clock mode 2
  void showMovingTime(const struct tm *tm)
  {
    // relative offsets showTime() writes with showSeconds=true, space=true (+2/+5 are ":" gaps)
    constexpr uint8_t MOVING_TIME_OFFSETS[6] = {0, 1, 3, 4, 6, 7};

    if (tm->tm_sec != _movingLastSecond)
    {
      int oldPosition = _movingPosition;
      _refreshLighting = true;
      _movingLastSecond = tm->tm_sec;
      ...
      {
        int final = 0;
        if ((SettingsCache::leadingZero == leading_zero::off) && ((tm->tm_hour) / 10 == 0))
        {
          final = (-1);
        }
      ...
      // blank every tube written in the old window but not in the new one, so nothing lingers
      for (uint8_t offset : MOVING_TIME_OFFSETS)
      {
        int oldPos = oldPosition + offset;
        bool stillUsed = false;
        for (uint8_t newOffset : MOVING_TIME_OFFSETS)
        {
          if (_movingPosition + newOffset == oldPos)
          {
            stillUsed = true;
            break;
          }
        }
        if (!stillUsed && (oldPos >= 0) && (oldPos < _displayHandler->getDigitCount()))
        {
          _displayHandler->setDigit(oldPos, DIGIT_OFF);
        }
      }
    }
    _displayHandler->showTime(tm, _movingPosition, SettingsCache::hourMode, SettingsCache::leadingZero, true, true, SettingsCache::timeSeparator);
  }

  // display date or time, clock mode 3
  void showDateOrTime(const struct tm *tm)
  {
    if ((tm->tm_sec > _showDateSecond) && (tm->tm_sec < _showDateSecond + 5))
    {
      if (!_scrollingIsDate)
      {
        if (millis() - _scrollingLastMillis > 75)
        {
          ...
          _displayHandler->showTime(tm, _scrollingTimePosition, SettingsCache::hourMode, SettingsCache::leadingZero, true, true, SettingsCache::timeSeparator);
  ...
  void showTimeAndDate(const struct tm *tm) const
  {
    _displayHandler->showTime(tm, 0, SettingsCache::hourMode, SettingsCache::leadingZero, true, false, SettingsCache::timeSeparator);
    _displayHandler->showDate(tm, 8, SettingsCache::dateFormat, year_type::partial, false);
  }

  // display time and temperature, clock mode 5
  void showTimeAndTemp(const struct tm *tm)
  {
    _displayHandler->showTime(tm, 0, SettingsCache::hourMode, SettingsCache::leadingZero, true, true, SettingsCache::timeSeparator);
    showTemp(11);
  }

  // display time, date and temperature, clock mode 6
  void showTimeAndDateAndTemp(const struct tm *tm)
  {
    _displayHandler->showTime(tm, 0, SettingsCache::hourMode, SettingsCache::leadingZero, false, false, SettingsCache::timeSeparator);
  ...
  void showDateTimeRaw(const struct tm *tm) const
  {
    ...
    uint8_t hours10 = tm->tm_hour / 10;
    uint8_t hours01 = tm->tm_hour % 10;
  ...
  // display local and dual time, clock mode 8
  void showDualTime(const struct tm *tm, const struct tm *tm2) const
  {
    _displayHandler->showTime(tm, 0, SettingsCache::hourMode, SettingsCache::leadingZero, true, false, SettingsCache::timeSeparator);
    _displayHandler->showTime(tm2, 8, SettingsCache::hourMode, SettingsCache::leadingZero, true, false, SettingsCache::timeSeparator, true);
  }

  // display the timer, clock mode 9
  void showTimer()
  {
    uint8_t position = 1;
    ...
    checkElapsedColorChangeTrigger(_timerMode == timer_mode::running, ts, tm, th,
  ...
  void showStopWatch()
  {
    ...
    checkElapsedColorChangeTrigger(_swRunning, ts, tm, th,
                                   _lastStopwatchColorSecond, _lastStopwatchColorMinute, _lastStopwatchColorHour);
  }

  // display blinking user input for the timer
  void showInput()
  {
    if (SettingsCache::inputBlinking == input_blinking::on)
    {
