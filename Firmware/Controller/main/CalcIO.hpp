...
public:
  ...
  {
    // get string in scientific format with the max available display precision
    // plus 2 extra digits for later rounding
...
private:
  ...
  // return how many fractional digits fit on the display
  int availableFracDigits(int intPartLength) const
  {
    int available = static_cast<int>(_digitCount) - intPartLength;
    if (!_number.exponent.isEmpty())
    {
      // reduce by the exponent digit count plus 1 blank as a visual gap
      available -= static_cast<int>(_number.exponent.length() + 1);
    }
  ...
  {
    // round base
    int decimals = availableFracDigits(static_cast<int>(intPart.length()));
    if (fixedDecimals != 0)
    {
    ...
    bool carry = roundNumber(intPart, fracPart, decimals);
    ...
    if (fixedDecimals != 0)
    {
      ...
      int maxFracLength = availableFracDigits(static_cast<int>(intPart.length()));
  ...
  // round to n decimal places according to the selected rounding mode
  bool roundNumber(String &intPart, String &fracPart, int decimals)
  {
    bool carry = (SettingsCache::roundingMode != rounding_mode::cut) &&
                 (decimals < static_cast<int>(fracPart.length())) &&
                 (fracPart.charAt(decimals) >= '5');
  ...
  bool keepExponent(String &intPart, String &fracPart, uint8_t fixedDecimals)
  {
    ...
    if (_number.exponentNegative)
    {
      ...
      if ((static_cast<int>(fracPart.length()) >= static_cast<int>(_digitCount) + exponent) && exponent < -3)
      {
        result = true;
      }
    }
  ...
  int getAvailableBaseDigits() const
  {
    int result = 0;
    if (_number.exponent.isEmpty())
    {
  ...
  int getAvailableExponentDigits() const
  {
    int result = 0;
  ...
  void printNumber()
  {
    __serial_println("Base:          " + _number.base);
    __serial_println("Base sign:     " + String(_number.baseNegative ? "-" : "+"));
    __serial_println("Exponent:      " + _number.exponent);
    __serial_println("Exponent sign: " + String(_number.exponentNegative ? "-" : "+"));
  }
};
