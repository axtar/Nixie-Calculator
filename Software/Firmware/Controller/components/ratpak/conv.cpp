...
static constexpr string_view DIGITS = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_@";
...

{
  ...
  if (SUCCEEDED(Calc_ULongAdd(size, 1, &cbAlloc)) && SUCCEEDED(Calc_ULongMult(cbAlloc, sizeof(MANTTYPE), &cbAlloc)) && SUCCEEDED(Calc_ULongAdd(cbAlloc, sizeof(NUMBER), &cbAlloc)))
  {
...
PRAT StringToRat(bool mantissaIsNegative, string_view mantissa, bool exponentIsNegative, string_view exponent, uint32_t radix, int32_t precision)
{
...
char NormalizeCharDigit(char c, uint32_t radix)
{
  // Allow upper and lower case letters as equivalent, base
  // is in the range where this is not ambiguous.
  if (size_t{radix} >= DIGITS.find('A') && size_t{radix} <= DIGITS.find('Z'))
  {
    return towupper(c);
  }
...
PNUMBER StringToNumber(string_view numberString, uint32_t radix, int32_t precision)
{
  ...
  for (const auto &c : numberString)
  {
    ...
    char curChar = (c == g_decimalSeparator ? '.' : c);
    ...
    case DD:
    {
      ...
      size_t pos = DIGITS.find(curChar);
      if (pos != string_view::npos && pos < static_cast<size_t>(radix))
      {
...
//
//-----------------------------------------------------------------------------
string NumberToString(PNUMBER &pnum, NumberFormat format, uint32_t radix, int32_t precision)
{
  ...
  {
    // fix showing 0 as 0.e-1 instead of 0.e+0
...
//
//-----------------------------------------------------------------------------
string RatToString(PRAT &prat, NumberFormat format, uint32_t radix, int32_t precision)
{
  PNUMBER p = RatToNumber(prat, radix, precision);

  string result = NumberToString(p, format, radix, precision);
  destroynum(p);
...
//-----------------------------------------------------------------------------
//
//  FUNCTION: RatToScientificString
//
//  ARGUMENTS:
//              PRAT *representation of a number.
//              uint32_t radix
...
//  RETURN: string
//
//  DESCRIPTION: returns a string representation in scientific format of rational number passed
//  in without rounding
//
//-----------------------------------------------------------------------------
string RatToScientificString(PRAT &prat, uint32_t radix, int32_t precision)
{
  PNUMBER p = RatToNumber(prat, radix, precision);

  string result = NumberToScientificString(p, radix, precision);
  destroynum(p);
...
//-----------------------------------------------------------------------------
//
//    FUNCTION: NumberToScientificString
//
//    ARGUMENTS: number representation
//          integer radix and int32_t precision value
//
//    RETURN: String
//
//    DESCRIPTION: Converts a number to its scientific string
//    representation without rounding
//
//-----------------------------------------------------------------------------
string NumberToScientificString(PNUMBER &pnum, uint32_t radix, int32_t precision)
{
  stripzeroesnum(pnum, precision + 2);
  int32_t length = pnum->cdigit;
  int32_t exponent = pnum->exp + length; // Actual number of digits to the left of decimal
...
//
//    ARGUMENTS: number representation
//               uint32_t radix value
//               int32_t precision value
//
//    RETURN: none
//
...
//
//-----------------------------------------------------------------------------
void roundnum(PNUMBER *pnum, uint32_t radix, int32_t precision, int32_t count)
{
  stripzeroesnum(*pnum, precision + 2);
  int32_t length = (*pnum)->cdigit;
  int32_t exponent = (*pnum)->exp + length; // Actual number of digits to the left of decimal
  ...
  if (round != nullptr)
  {
    ...
    if (stripzeroesnum(*pnum, offset))
    {
      ...
      if (count < 3)
      {
        return roundnum(pnum, radix, precision, ++count);
      }
    }
  }
  else
...
//    RETURN: none
//
//    DESCRIPTION: cut digits to precision
//
//-----------------------------------------------------------------------------

void cutdigits(PNUMBER *pnum, int32_t precision)
{
  if (!zernum(*pnum) && ((*pnum)->cdigit > precision) && ((*pnum)->exp < 0))
  {
    MANTTYPE *pmant = (*pnum)->mant;
    int32_t cdigits = (*pnum)->cdigit;
    int32_t diff = (*pnum)->cdigit - precision;
    pmant += diff;
    cdigits = precision;
    memmove((*pnum)->mant, pmant, (int)(cdigits * sizeof(MANTTYPE)));
    (*pnum)->exp += ((*pnum)->cdigit - cdigits);
